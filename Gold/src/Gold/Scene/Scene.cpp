...
namespace Gold {
	...
	void Scene::OnUpdateRuntime(Timestep ts)
	{
		...
		if (mainCamera)
		{
			...
			for (auto entity : view)
			{
				auto [Animation, sprite] = view.get<AnimationComponent, SpriteRendererComponent>(entity);

				if(Animation.AnimationTextureSlot.size()!=0)
					sprite.Texture = Animation.AnimationTextureSlot[Animation.AnimIndex];

				Animation.Update(ts);
			}

			Renderer2D::BeginScene(*mainCamera, cameraTransform,ts);
	...
	{
		Renderer2D::BeginScene(camera,ts);
	...
	void Scene::OnComponentAdded<AnimationComponent>(Entity entity, AnimationComponent& component)
	{
		if (entity.HasComponent<SpriteRendererComponent>())
		{
			...
			if (SpriteRendererComp.Texture != nullptr)
				component.AnimationTextureSlot.push_back(SpriteRendererComp.Texture);
		}
	}
}