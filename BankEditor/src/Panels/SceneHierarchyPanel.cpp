...
namespace Gold {
	...
	void SceneHierarchyPanel::DrawComponents(Entity entity)
	{
		...
		if (ImGui::BeginPopup("AddComponent"))
		{
			...
			if (!m_SelectionContext.HasComponent<AnimationComponent>() && m_SelectionContext.HasComponent<SpriteRendererComponent>())
			{
		...
		DrawComponent<SpriteRendererComponent>("Sprite Renderer", entity, [](auto& component)
		{
			...
			if (ImGui::BeginDragDropTarget())
			{
				if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
				{
					...
					component.Texture = Texture2D::Create(texturePath.string(),false, true);
				}
		...
		DrawComponent<AnimationComponent>("Animation", entity, [](auto& component)
		{
			...
			if (ImGui::BeginDragDropTarget())
			{
				if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
				{
					const wchar_t* path = (const wchar_t*)payload->Data;
					std::filesystem::path texturePath = std::filesystem::path(g_Assetspath) / path;
					component.AnimationTextureSlot.push_back(Texture2D::Create(texturePath.string(),false, true));
				}
